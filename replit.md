...
- Files are workspace-scoped, persisted assets. Schema: `datasets` (per file, with `issues` jsonb storing per-issue `status`), `dataset_columns` (typed metadata + overrides), `dataset_rows` (sampled rows blob, capped at 100k). Settings table gained `readiness_threshold` (default 60).
- Server (`/api/workspaces/:id/datasets`, `/api/datasets/:id`, column PATCH, issue PATCH, dataset DELETE — all in DB transactions) parses workbook → classifies columns into semantic types (date/currency/percent/id/category/measure/boolean/text) → scores readiness 0-100 by penalising 7 issue categories (missing values, duplicates, format inconsistency, **outliers via 1.5×IQR on numeric columns**, invalid dates, empty columns, suspicious negatives). Each issue has a `status` (open/ignored/resolved/review) which persists across recompute and refunds the score for ignored/resolved entries. Dataset detail also returns `suggestedKpis` — server-side KPI suggestions derived from semantic types (count_distinct on IDs, sum/avg on currency, avg on percent/measure, breakdown on small categoricals).
- UI: WorkspaceDetail Files tab hosts a drag-and-drop zone, **fast-sample banner** when any dataset was truncated, dataset list with score badges, and per-dataset detail with two sub-tabs — Understanding (AI-suggested KPI chips, column chips, semantic-type override Select, business-meaning Input, sample rows) and Quality (radial score gauge with workspace threshold badge, grouped issue cards with severity + status badges and per-issue **Auto-fix / Ignore / Review / Reopen** actions, ignored/resolved hidden by default with "Show all" toggle, client-side CSV issue export, "Continue to Join Studio" CTA wired to navigate to `/workspaces/:id/prepared` once score ≥ workspace threshold, and a "Continue anyway" soft override that confirms then navigates).
- Legacy `/upload` route redirects to the most recent workspace's `/files` tab via wouter `<Redirect>`; the previous direct-generate page is preserved at `/upload/legacy`.
