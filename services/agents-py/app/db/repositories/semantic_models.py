...
async def latest_raw(project_id: int) -> dict[str, Any] | None:
    return await db.fetch_one(
        f"SELECT * FROM {TABLE} WHERE project_id = %s ORDER BY created_at DESC LIMIT 1",
        [project_id],
    )
...
async def get_applied_raw(project_id: int) -> dict[str, Any] | None:
    return await db.fetch_one(
        f"SELECT * FROM {TABLE} WHERE project_id = %s AND status = 'applied' "
        f"ORDER BY created_at DESC LIMIT 1",
        [project_id],
    )
...
async def delete_proposed(project_id: int) -> None:
    await db.execute(
        f"DELETE FROM {TABLE} WHERE project_id = %s AND status = 'proposed'", [project_id]
    )


async def insert_proposed(project_id: int, graph_definition: dict[str, Any], rationale: str) -> dict[str, Any]:
    row = await db.fetch_one(
        f"INSERT INTO {TABLE} (project_id, status, graph_definition, agent_rationale) "
...
async def demote_applied(project_id: int) -> None:
    await db.execute(
        ...
        f"WHERE project_id = %s AND status = 'applied'",
...
async def set_status(project_id: int, sm_id: int, status: str) -> dict[str, Any] | None:
    row = await db.fetch_one(
        f"UPDATE {TABLE} SET status = %s, updated_at = %s WHERE id = %s AND project_id = %s RETURNING *",
...
async def delete(project_id: int, sm_id: int) -> None:
    await db.execute(f"DELETE FROM {TABLE} WHERE id = %s AND project_id = %s", [sm_id, project_id])
