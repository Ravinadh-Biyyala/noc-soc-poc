...
async def list_by_project(project_id: int) -> list[dict[str, Any]]:
    rows = await db.fetch_all(
        f"SELECT * FROM {TABLE} WHERE project_id = %s ORDER BY created_at DESC", [project_id]
    )
...
async def existing_summaries(project_id: int) -> list[dict[str, Any]]:
    rows = await db.fetch_all(
        f"SELECT metric_name, sql_formula, status FROM {TABLE} WHERE project_id = %s "
...
async def get_by_name(project_id: int, metric_name: str) -> dict[str, Any] | None:
    return await db.fetch_one(
        f"SELECT * FROM {TABLE} WHERE project_id = %s AND metric_name = %s LIMIT 1",
...
async def insert(
    ...
    row = await db.fetch_one(
        f"INSERT INTO {TABLE} "
        f"(project_id, metric_name, description, sql_formula, depends_on_tables, status, agent_rationale) "
...
async def set_status(project_id: int, mid: int, status: str) -> dict[str, Any] | None:
    row = await db.fetch_one(
        f"UPDATE {TABLE} SET status = %s WHERE id = %s AND project_id = %s RETURNING *",
...
async def patch(project_id: int, mid: int, updates: dict[str, Any]) -> dict[str, Any] | None:
    if not updates:
        row = await db.fetch_one(
            f"SELECT * FROM {TABLE} WHERE id = %s AND project_id = %s", [mid, project_id]
        )
        return mappers.metric(row) if row else None
    ...
    row = await db.fetch_one(
        f"UPDATE {TABLE} SET {', '.join(cols)} WHERE id = %s AND project_id = %s RETURNING *",
        vals,
    )
    return mappers.metric(row) if row else None
...
async def delete(project_id: int, mid: int) -> None:
    await db.execute(f"DELETE FROM {TABLE} WHERE id = %s AND project_id = %s", [mid, project_id])
