    ...
    <div className="flex h-full min-h-0 animate-in fade-in duration-300">
      ...
      {addingOp && (
        <OperationModal
          type={addingOp}
          tables={tables}
          seedJoin={seedJoinSuggestion}
          onClose={() => { setAddingOp(null); setSeedJoinSuggestion(null); }}
          onAdd={(op) => { addOperation(op); setSeedJoinSuggestion(null); }}
        />
      )}
    </div>
  );
...
function OperationModal({ type, tables, seedJoin, onClose, onAdd }: {
  type: Exclude<AddingOpType, null>;
  tables: Table[];
  seedJoin?: JoinSuggestion | null;
  onClose: () => void;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 animate-in fade-in duration-150">
      <Card className="w-full max-w-lg shadow-2xl animate-in zoom-in-95 duration-200">
        <CardContent className="p-5 space-y-4">
          ...
          {type === "join" && <JoinForm tables={tables} seedJoin={seedJoin} onAdd={onAdd} />}
...
function JoinForm({ tables, seedJoin, onAdd }: { tables: Table[]; seedJoin?: JoinSuggestion | null; onAdd: (op: JoinOperation) => void }) {
  const [leftId, setLeftId] = useState(seedJoin?.leftTableId || tables[0]?.id || "");
  const [rightId, setRightId] = useState(seedJoin?.rightTableId || tables[1]?.id || "");
  const [leftKey, setLeftKey] = useState(seedJoin?.leftKey || "");
  const [rightKey, setRightKey] = useState(seedJoin?.rightKey || "");
